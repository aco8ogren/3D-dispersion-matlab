function [T,dTdwavevector] = get_transformation_matrix(wavevector,const)
    ...
    unreduced_node_numbers_grid = reshape(1:number_of_nodes_full,N_node);
    ...
    reduced_node_numbers_grid = reshape(1:number_of_nodes_reduced,N_node_reduced);
    ...
    nonagent_phase_vector = ones(N_dof_per_node*numel(nonagent_unreduced_node_numbers_grid),1);
    xphase_vector = xphase*ones(N_dof_per_node*numel(x_agent_node_numbers_grid),1);
    yphase_vector = yphase*ones(N_dof_per_node*numel(y_agent_node_numbers_grid),1);
    zphase_vector = zphase*ones(N_dof_per_node*numel(z_agent_node_numbers_grid),1);
    yzphase_vector = yzphase*ones(N_dof_per_node*numel(yz_agent_node_numbers_grid),1);
    xzphase_vector = xzphase*ones(N_dof_per_node*numel(xz_agent_node_numbers_grid),1);
    xyphase_vector = xyphase*ones(N_dof_per_node*numel(xy_agent_node_numbers_grid),1);
    xyzphase_vector = xyzphase*ones(N_dof_per_node*numel(xyz_agent_node_numbers_grid),1);
